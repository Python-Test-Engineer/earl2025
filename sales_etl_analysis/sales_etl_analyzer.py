...
class SalesETLAnalyzer:
    ...
    def transform_data(self):
        ...
        if (
            "quantity" in self.clean_data.columns
            and "unit_price" in self.clean_data.columns
        ):
            self.clean_data["total_sales"] = (
                self.clean_data["quantity"] * self.clean_data["unit_price"]
            )
        ...
        print(
            f"✅ Data transformation complete. Final dataset: {len(self.clean_data)} records"
        )
    ...
    def analyze_data(self):
        ...
        if "date" in self.clean_data.columns:
            self.analysis_results["daily_sales"] = (
                self.clean_data.groupby("date")["total_sales"].sum().round(2)
            )

        # Monthly sales trend
        if "year" in self.clean_data.columns and "month" in self.clean_data.columns:
            self.analysis_results["monthly_sales"] = (
                self.clean_data.groupby(["year", "month"])["total_sales"].sum().round(2)
            )
    ...
    def create_visualizations(self):
        ...
        if "category" in self.clean_data.columns:
            ...
            plt.tight_layout()
            plt.savefig(
                self.plots_dir / "sales_by_category.png", dpi=300, bbox_inches="tight"
            )
            plt.close()
        ...
        if "region" in self.clean_data.columns:
            ...
            plt.tight_layout()
            plt.savefig(
                self.plots_dir / "sales_by_region_pie.png", dpi=300, bbox_inches="tight"
            )
            plt.close()
        ...
        if "date" in self.clean_data.columns:
            ...
            plt.tight_layout()
            plt.savefig(
                self.plots_dir / "daily_sales_trend.png", dpi=300, bbox_inches="tight"
            )
            plt.close()
        ...
        if "product_name" in self.clean_data.columns:
            ...
            plt.tight_layout()
            plt.savefig(
                self.plots_dir / "top_products.png", dpi=300, bbox_inches="tight"
            )
            plt.close()
        ...
        if "sales_rep" in self.clean_data.columns:
            ...
            plt.title(
                "Sales Representative Performance", fontsize=16, fontweight="bold"
            )
            plt.xlabel("Total Sales ($)", fontsize=12)
            ...
            plt.tight_layout()
            plt.savefig(
                self.plots_dir / "sales_rep_performance.png",
                dpi=300,
                bbox_inches="tight",
            )
            plt.close()
        ...
        if (
            ...
            plt.figure(figsize=(10, 6))
            plt.scatter(
                self.clean_data["unit_price"],
                self.clean_data["quantity"],
                alpha=0.6,
                c="purple",
                s=50,
            )
            plt.title(
                "Quantity vs Unit Price Relationship", fontsize=16, fontweight="bold"
            )
            ...
            plt.tight_layout()
            plt.savefig(
                self.plots_dir / "quantity_vs_price_scatter.png",
                dpi=300,
                bbox_inches="tight",
            )
            plt.close()
        ...
        if (
            "category" in self.clean_data.columns
            and "region" in self.clean_data.columns
        ):
            plt.figure(figsize=(10, 6))
            ...
            plt.title(
                "Sales Heatmap: Category vs Region", fontsize=16, fontweight="bold"
            )
            plt.tight_layout()
            plt.savefig(
                self.plots_dir / "sales_heatmap_category_region.png",
                dpi=300,
                bbox_inches="tight",
            )
            plt.close()
    ...
    def generate_report(self):
        ...
        report_file = (
            self.reports_dir
            / f"sales_analysis_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
        )
...
def main():
    ...
    if os.path.exists(sample_file):
        ...
        print("\nExpected CSV format:")
        print(
            "date,product_id,product_name,category,quantity,unit_price,customer_id,customer_name,region,sales_rep"
        )
