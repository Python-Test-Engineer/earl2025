...
FILE_PATH = "./sales_etl_analysis/sample_sales_data.csv"
def demo_full_pipeline():
    ...
    if os.path.exists(FILE_PATH):
        analyzer.run_full_etl_analysis(FILE_PATH)
...
def demo_step_by_step():
    ...
    if os.path.exists(FILE_PATH):
        # Step 1: Extract
        print("\n1️⃣ EXTRACT")
        analyzer.extract_data(FILE_PATH)
...
def demo_custom_analysis():
    ...
    if os.path.exists(FILE_PATH):
        ...
        top_products = (
            data.groupby("product_name")["total_sales"]
            .sum()
        ...
        avg_by_region = (
            data.groupby("region")["total_sales"].mean().sort_values(ascending=False)
        )
        ...
        rep_performance = (
            data.groupby("sales_rep")["total_sales"].agg(["sum", "count"]).round(2)
        )
