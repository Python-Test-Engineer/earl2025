...
def generate_sample_data(rows: int = 1000) -> str:
    ...
    df.loc[df.sample(int(0.02 * rows)).index, "sales_amount"] = (
        np.nan
    )  # 2% missing values
    ...
    df.loc[df.sample(int(0.01 * rows)).index, "region"] = (
...
def generate_insights(data_json: str) -> str:
    ...
    try:
        ...
        if "region" in df.columns and "sales_amount" in df.columns:
            region_sales = (
                df.groupby("region")["sales_amount"].mean().sort_values(ascending=False)
            )
            best_region = region_sales.index[0]
            worst_region = region_sales.index[-1]
            insights.append(
                f"Best performing region: {best_region} (${region_sales[best_region]:.0f} avg)"
            )
            insights.append(
                f"Lowest performing region: {worst_region} (${region_sales[worst_region]:.0f} avg)"
            )

        # Product performance
        if "product" in df.columns:
            product_sales = (
                df.groupby("product")["sales_amount"].sum().sort_values(ascending=False)
            )
            top_product = product_sales.index[0]
            insights.append(
                f"Top selling product: {top_product} (${product_sales[top_product]:,.0f} total)"
            )
        ...
        if "date" in df.columns:
            ...
            insights.append(
                f"Peak sales month: {peak_month} (${monthly_sales[peak_month]:.0f} avg)"
            )
        ...
        if "customer_satisfaction" in df.columns:
            ...
            insights.append(
                f"Average customer satisfaction: {satisfaction_avg:.2f}/5.0"
            )
...
def create_data_ingestion_agent():
    ...
    def agent_node(state: AgentState):
        ...
        state["messages"].append(
            HumanMessage(content="Generate sample sales data for analysis")
        )
...
def create_analytics_agent():
    ...
    def agent_node(state: AgentState):
        ...
        insights.append(
            f"💰 Top region: {region_sales.index[0]} (${region_sales.iloc[0]:.0f} avg)"
        )
        ...
        insights.append(
            f"📱 Top product: {product_sales.index[0]} (${product_sales.iloc[0]:,.0f} total)"
        )
...
def create_reporting_agent():
    ...
    def agent_node(state: AgentState):
        ...
        fig.suptitle(
            "🤖 AI Agent Pipeline Analysis Dashboard", fontsize=16, fontweight="bold"
        )
        ...
        ax4.scatter(
            df["customer_satisfaction"], df["sales_amount"], alpha=0.6, color="coral"
        )
...
def main():
    ...
    try:
        ...
        print(
            f"📊 Data Quality Score: {final_state['quality_report'].get('quality_score', 'N/A')}%"
        )
        print(f"🔍 Insights Generated: {len(final_state.get('insights', []))}")
        print(
            f"📈 Visualizations Created: {len(final_state.get('visualizations', []))}"
        )
        ...
        print(
            "💡 Install requirements: pip install langgraph langchain-openai pandas numpy matplotlib seaborn"
        )
